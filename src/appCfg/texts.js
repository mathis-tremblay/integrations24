import {appConsts} from "./appConsts";

export const texts = {

    //TODO:
    info: {
        bienvenue: <p>
            <p>Le monde a changé. Je le vois dans l’eau, je le ressens dans la terre, je le sens dans l’air. Beaucoup de
                ce
                qui existait jadis est perdu, car aucun de ceux qui vivent aujourd’hui ne s’en souvient. Tout commença
                lorsque le pont de Québec tomba en 1907. De grands anneaux furent forgés. Trois furent donnés aux
                ingénieurs
                informatiques et électriques, les plus sages et les plus respectables de tous les êtres. Sept, aux
                ingénieurs minier et chimique, grands mineurs et fabriquant du nord. Et neuf, neuf anneaux furent donnés
                aux
                ingénieurs civils et mécaniques qui par-dessus tout désiraient le pouvoir, car à travers ces anneaux
                furent
                transmises la force et la volonté de construire de grandes choses. Mais ils furent tous dupés, car un
                autre
                anneau fut forgé. </p>
            <br/>
            <p>Sous le fleuve Saint-Laurent, avec les fragments du pont de Québec, les septs gardiens, Seigneurs des
                ingénieurs forgèrent en secret un maître anneau pour gouverner tous les autres. Dans cet anneau, ils
                déversèrent leurs cruautés, leurs malveillances et leurs volontés d’empêcher tout autre d’obtenir des
                anneaux de pouvoir. Un anneau pour les gouverner tous. Le pouvoir de l’anneau ne pouvait être vaincu.
                Professeurs furent séduits, tous furent assujettis par l'Anneau Unique, et eux devinrent des spectres,
                des
                Ombres errant parmi les Ombres sous l'obéissance des septs gardiens. Ils sont les Nazgûls, les Spectres
                de
                l'Anneau, les serviteurs les plus terrifiants de l'Ennemi; les ténèbres les suivent, et ils hurlent de
                terreur à la seule pensée de faire couler un étudiant en ingénierie. </p>
            <br/>
            <p>Et certaines choses qui n’auraient pas dû être oubliées furent perdues. L’histoire devint une légende, la
                légende devint un mythe. Jusqu’à ce que par hasard il fut entrepris par la créature la plus improbable
                qui
                soit : TOI chère Gelgifois. En effet, l’heure approche où vous déterminerez le destin de tous si vous
                vous
                montrez capable de réussir le bacc en génie.</p>
        </p>,
        infoGenerales: <p>
            <p>
                Félicitations pour ton admission en génie électrique/informatique à l'Université Laval ! Pour commencer
                la plus belle aventure de ton parcours scolaire, tu es invité à nous joindre pour les intégrations qui
                se dérouleront du lundi 2 septembre au vendredi 6 septembre. Ces activités auront pour but de forger un
                lien d’appartenance au baccalauréat, de rencontrer ceux avec qui tu partageras de bons souvenirs et de
                moins bonnes notes en plus des braves qui sont passés par là avant toi. Nous en profiterons
                aussi pour te faire connaître toutes les ressources qui sont à ta disposition pour t’aider dans tes
                études et te permettre de vivre la meilleure des vies universitaires !
                Pour t'inscrire aux activités, coche la case <em>Je participe</em> dans l'onglet Horaire.
            </p>
            <p>
                Cependant, pour participer à ces activités, tu dois payer ta contribution
                de <b>{appConsts.infos.amountToPay}$</b> à <em>trésorier@gelgif.com</em> (en minuscule).
            </p>
            <p>
                <b>Question :</b> <span style={{fontWeight: "normal"}}>integration</span> <br/>
                <b>Réponse :</b> <span style={{fontWeight: "normal"}}>gelgif</span> <br/>
                <span style={{fontWeight: "normal"}}>Ça vaut la peine, je te le garantis !</span> <br/> <br/>
            </p>
            <p>
                Lors des intégrations, sois assuré que tu as le droit de refuser tout ce qui te semble inacceptable.
                L'objectif n'est pas de rabaisser personne, mais bien d'apprendre à se connaître et à apprivoiser ton
                environnement pour les 4 (et plus) prochaines années. De plus, tous les membres du comité ont préalablement
                suivi la formation sur la prévention des violences de nature sexuelle dans des activités sociales et la
                formation sur la consommation d’alcool lors d’activités à l’Université Laval. Ton bien-être nous tient à
                coeur et nous souhaitons que les activités d’intégration restent agréables et amusantes !
            </p>
            <p>
                Finalement, va chercher ta LPU (laissez-passer universitaire). C'est important pour suivre le groupe
                sans avoir à payer tous les trajets de bus. Les informations sont sur le MonPortail.
            </p>
            <br/>
            <p>
                N'hésite pas à nous écrire dans la section Messages si tu as des questions !
            </p>
            <b>Le comité intégrateur 2024</b> <br/>
            <b>GelGif</b>
        </p>,
        infoImportantes: <p>
            <p>Les intégrations se déroulent du <span style={{fontWeight: "bold"}}>2 au 6 septembre 2024</span>.</p>
            <p><span style={{fontWeight: "bold"}}>AUCUNE ACTIVITÉ N'EST OBLIGATOIRE !</span> Le but des intégrations est
                de te permettre de connaitre les personnes que tu vas côtoyer pour tout ton baccalauréat et d'avoir du
                fun. Tu as le droit de refuser quoi que ce soit qui te rend mal à l'aise. La consommation d'alcool
                n'est jamais imposée.
            </p>
            <p>Pour participer aux activités, coche la case <em>Je participe</em> dans l'onglet Horaire pour chaque
                activité que tu souhaites rejoindre. Par contre, pour participer, il faut <b>payer</b> !</p>
            <p>Tu peux amener tes consommations, mais AUCUN contenant en verre n'est permis.</p>
            <p>N'oublie pas ta <b>LPU</b> !</p>
            <b>Le comité intégrateur 2024</b> <br/>
            <b>GelGif</b>
        </p>,
        frais: <p>
            <p>Les frais d'intégration sont de <b>{appConsts.infos.amountToPay}$</b>. Ils couvrent la grande majorité
                des coûts pour les activités de la semaine.</p>
            <p><em>Pas de paiement = Pas d'intégration !</em> Donc assure-toi de payer ta contribution
                de <b>{appConsts.infos.amountToPay}$</b> à <em>trésorier@gelgif.com</em> (en minuscule) avant le début des
                intégrations,
                soit lundi le 2 septembre.</p>
            <b>Question : </b> <span style={{fontWeight: "normal"}}>integration</span> <br/>
            <b>Réponse : </b> <span style={{fontWeight: "normal"}}>gelgif</span> <br/>
            <br/>
            <p><b>Les frais couvrent les activités suivantes:</b>
                <p>
                    <p style={{margin: 0, marginLeft: 5}}>- Votre magnifique chandail des intégrations</p>
                    <p style={{margin: 0, marginLeft: 5}}>- Dîner du lundi</p>
                    <p style={{margin: 0, marginLeft: 5}}>- Entrée à la Petite Grenouille lundi soir</p>
                    <p style={{margin: 0, marginLeft: 5}}>- Les consommations du mardi</p>
                    <p style={{margin: 0, marginLeft: 5}}>- Des consommations <em>ILLIMITÉES</em> vendredi</p>
                    <p style={{margin: 0, marginLeft: 5}}>- Le transport le vendredi</p>
                </p>
            </p>
            <p><b>Pas inclus dans les frais:</b>
                <p>
                    <p style={{margin: 0, marginLeft: 5}}>- Les consommations à la Petite Grenouille le lundi</p>
                    <p style={{margin: 0, marginLeft: 5}}>- Les consommations du mercredi</p>
                    <p style={{margin: 0, marginLeft: 5}}>- Le souper du jeudi</p>
                </p>
            </p>
            <br/>
            <p>N'hésite pas à nous écrire dans la section Messages si tu as des questions!</p>
            <b>Le comité intégrateur 2024</b> <br/>
            <b>GelGif</b>
        </p>
    },

    //TODO:
    day: {
        one: <p style={{
            display: "flex",
            flexDirection: "column",
            justifyContent: "center",
            alignItems: "center",
        }}>
            Bla
        </p>,
        two: <p style={{
            display: "flex",
            flexDirection: "column",
            justifyContent: "center",
            alignItems: "center",
        }}>
            <b>17h00 - On va t'occuper ⚽️</b>
            <p>
            On se retrouve sur le Grand Axe pour souper (amène un lunch) et jouer à des jeux genre spikeball et frisbee.
            </p>
            <b>19h00 - Rallye d'apparts 🥴</b>
            <>"It's not about being the best, but about being better than you were yesterday" - George St-Pierre</>
            <p>Bref, on part faire la tournée de quelques apparts et s'amuser comme des p'tits fous.</p>
            <b>23h00 - After pour les plus crinqués 🕺</b>
            <p>Si t'as encore de l'énergie, suis-nous.</p>
            <b>??h?? - Chu chaud, j'veux une poutine 🍟</b>
            <p>Pour ceux qui veulent finir ça en beauté --> au Salvatore!</p>
        </p>,
        three: <p style={{
            display: "flex",
            flexDirection: "column",
            justifyContent: "center",
            alignItems: "center",
        }}>
            BlaBlaBla
        </p>,
        four: <p style={{
            display: "flex",
            flexDirection: "column",
            justifyContent: "center",
            alignItems: "center",
        }}>
            BlaBlaBlaBla
        </p>,
        fiveSurprise: <p style={{
            display: "flex",
            flexDirection: "column",
            justifyContent: "center",
            alignItems: "center",
        }}>
            <b>18h00 - ❓❓❓</b>
            <p>Rejoins-nous dans le parking en face du pavillon Desjardins... Crois nous, ça vaut la peine.</p>
        </p>,
        fiveComplete: <p style={{
            display: "flex",
            flexDirection: "column",
            justifyContent: "center",
            alignItems: "center",
        }}>
            <b>18h00 - Départ vers le chalet 🚌</b>
            <p>Rejoins-nous dans le parking en face du pavillon Desjardins. On s'en va en chalet faire le party!! La
                boisson est incluse, donc manque pas ça!</p>
            <b>19h00 - Souper roteux 🌭</b>
            <p>On vous sert des bons hot-dogs pour souper. On sait que tu aimes boire, mais c'est important de manger
                aussi.</p>
            <b>20h00 - PARTYYYYYY 🍻</b>
            <p>Le party commence! On a hâte de te voir sur le dance floor ou bien au beer pong.</p>
            <b>23h30 - Premier départ 👋</b>
            <p>Premier départ d'autobus. Si tu es tanné ou tu ne tiens plus debout, c'est le temps de partir.</p>
            <b>00h30 - Dernier départ 😴</b>
            <p>Dernier départ d'autobus. On est tous fatigués et c'est le temps de partir.</p>
        </p>,
    }
}